...
void iniciar_logs(){
	...
	if( logger == NULL){
        ...
        log_info (loggerError, "[IO] Error Logger inciado correctamente");
    }
...
void iniciar_configs(char* arg){
 ...
 log_info (logger, "antes de venir 2");
    ...
    BLOCK_COUNT = config_get_int_value(config, "BLOCK_COUNT");
     log_info (logger, "antes de venir 1");
}

void mostrar_configs(){
            log_info (logger, "antes de venir ");
